variable "api_url" {
  type        = string
  description = "Promox API URL"
}

variable "api_token" {
  type        = string
  description = "Promox API Token"
  sensitive   = true
}

variable "api_token_id" {
  type        = string
  description = "Promox API Token ID"
}

variable "cloud_init_user" {
  type        = string
  description = "Cloud Init User"
}

variable "cloud_init_passwd" {
  type      = string
  sensitive = true
}

variable "superkey" {
  type    = string
  default = <<EOF
    ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHSYMUEwIiZqpZPRlJNTAU2XubxaLfdRN7u0QsLyUVGF akbarfikriabdillah@gmail.com
    ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILhW+baFtRsOKa3XFEG91RLm8AAN5H//sw5s4euJyUSO yogarn@fedora
    ssh-rsa  msi@DESKTOP-57RMF98
  EOF
}

